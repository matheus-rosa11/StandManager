export type LanguageCode = 'pt-BR' | 'en-US' | 'es-ES';

export interface LanguageOption {
  code: LanguageCode;
  icon: string;
  labelKey: string;
}

export interface TranslateParams {
  count?: number;
  [key: string]: string | number | undefined;
}

type TranslationDictionary = Record<string, string>;

const STORAGE_KEY = 'stand-manager.language';
const DEFAULT_LANGUAGE: LanguageCode = 'pt-BR';

const LANGUAGE_OPTIONS: LanguageOption[] = [
  { code: 'pt-BR', icon: '🇧🇷', labelKey: 'language.pt-BR' },
  { code: 'en-US', icon: '🇺🇸', labelKey: 'language.en-US' },
  { code: 'es-ES', icon: '🇪🇸', labelKey: 'language.es-ES' }
];

const TRANSLATIONS: Record<LanguageCode, TranslationDictionary> = {
  'pt-BR': {
    'app.brand': 'Stand Manager',
    'footer.communityProject': 'Projeto comunitário - Gerenciador de pedidos de pastel',
    'nav.home': 'Início',
    'nav.volunteers': 'Pedidos Ativos',
    'nav.volunteerHistory': 'Histórico',
    'nav.cashier': 'Caixa',
    'nav.myOrders': 'Meus pedidos',
    'nav.selfService': 'Autoatendimento',
    'nav.admin': 'Administração',
    'nav.reports': 'Relatórios',
    'identity.headerBadge': 'Cliente #{{id}} — {{name}}',
    'identity.askRoleTitle': 'Você é voluntário do evento?',
    'identity.askRoleSubtitle': 'Precisamos dessa informação para liberar as telas corretas para você.',
    'identity.imVolunteer': 'Sou voluntário',
    'identity.imGuest': 'Sou cliente',
    'identity.hasIdentifierQuestion': 'Já tenho um número identificador',
    'identity.askIdentifierTitle': 'Você já possui um número identificador?',
    'identity.hasIdentifierYes': 'Sim, já tenho meu número',
    'identity.hasIdentifierNo': 'Ainda não tenho um número',
    'identity.enterIdentifierTitle': 'Digite seu número identificador',
    'identity.identifierPlaceholder': 'Ex: 42',
    'identity.loading': 'Carregando...',
    'identity.continue': 'Continuar',
    'identity.confirmNameTitle': 'Confirme seu nome — Cliente #{{id}}',
    'identity.confirmNameSubtitle': 'Seu nome cadastrado é {{name}}. Digite exatamente como está para confirmar.',
    'identity.namePlaceholder': 'Informe seu nome completo',
    'identity.confirmNameButton': 'Confirmar nome',
    'identity.identifierInvalid': 'Não encontramos esse número identificador. Tente novamente ou procure um voluntário.',
    'identity.nameRequired': 'Informe seu nome para continuar.',
    'identity.nameMismatch': 'O nome informado não confere com o cadastro. Procure um voluntário para ajudar.',
    'identity.enterNameTitle': 'Vamos criar seu identificador',
    'identity.enterNameSubtitle': 'Informe seu nome completo. Vamos gerar um número fácil para você lembrar.',
    'identity.createIdentifierButton': 'Gerar número identificador',
    'identity.alreadyHaveIdentifier': 'Já tenho um número identificador',
    'identity.registerFailed': 'Não foi possível gerar o identificador. Tente novamente.',
    'identity.customerWelcome': 'Beleza, você é o cliente número {{id}}, {{name}}!',
    'identity.persistReminder': 'Esse número ficará sempre no canto esquerdo do cabeçalho para você não esquecer.',
    'languageSelector.label': 'Selecionar idioma',
    'language.pt-BR': 'Português (Brasil)',
    'language.en-US': 'English (US)',
    'language.es-ES': 'Español',
    'volunteer.title': 'Painel dos voluntários',
    'volunteer.pendingItems': 'Itens pendentes',
    'volunteer.refresh': 'Atualizar agora',
    'volunteer.success': 'Pedido atualizado com sucesso.',
    'volunteer.genericError': 'Não foi possível atualizar o pedido.',
    'volunteer.loading': 'Carregando pedidos em andamento...',
    'volunteer.error': 'Erro ao carregar pedidos: {{message}}',
    'volunteer.empty': 'Não há pedidos pendentes no momento. 🎉',
    'volunteer.identifier': 'Cliente #{{id}}',
    'volunteer.searchPlaceholder': 'Filtrar por nome ou número do cliente',
    'volunteer.pendingPastels': '{{count}} pastel',
    'volunteer.pendingPastels_plural': '{{count}} pastéis',
    'volunteer.orderNumber': 'Pedido #{{order}}',
    'volunteer.receivedAt': 'Recebido em {{time}}',
    'volunteer.unitLabel': 'Unidade {{unit}} de {{total}}',
    'volunteer.advance': 'Avançar etapa',
    'volunteer.completed': 'Concluído',
    'volunteer.finalizedMessage': 'Pedido finalizado',
    'orderStatus.pending': 'Aguardando',
    'orderStatus.frying': 'Fritando',
    'orderStatus.readyForPickup': 'Pronto',
    'orderStatus.finalized': 'Pedido finalizado',
    'orderStatus.cancelled': 'Cancelado',
    'orderStatus.progress': 'Etapa {{step}} de {{total}}',
    'orderStatus.unknown': 'Etapa desconhecida',
    'customerOrder.title': 'Peça seu pastel',
    'customerOrder.subtitle':
      'Escolha os sabores disponíveis em tempo real e finalize seu pedido. Você será avisado quando estiver pronto! 😊',
    'customerOrder.customerBadge': 'Cliente #{{id}} — {{name}}',
    'customerOrder.identityMissing': 'Precisamos confirmar seu identificador com um voluntário antes de registrar pedidos.',
    'customerOrder.nameLabel': 'Seu nome',
    'customerOrder.namePlaceholder': 'Como devemos chamar você?',
    'customerOrder.availableFlavors': 'Sabores disponíveis',
    'customerOrder.refresh': 'Atualizar lista',
    'customerOrder.loading': 'Carregando sabores deliciosos...',
    'customerOrder.error': 'Não foi possível carregar os sabores: {{message}}',
    'customerOrder.inStock': 'Disponíveis: {{quantity}}',
    'customerOrder.itemsInCart': 'Itens no carrinho: {{count}}',
    'customerOrder.totalAmount': 'Total: {{value}}',
    'customerOrder.submit': 'Finalizar pedido',
    'customerOrder.submitting': 'Enviando...',
    'customerOrder.success': 'Pedido enviado! Você receberá uma notificação quando estiver pronto.',
    'customerOrder.validation': 'Selecione pelo menos um sabor para continuar.',
    'customerOrder.failure': 'Não foi possível registrar seu pedido. Tente novamente.',
    'cashier.title': 'Registro de pedidos - Caixa',
    'cashier.subtitle':
      'Consulte o estoque atualizado e selecione rapidamente os sabores solicitados pelos participantes.',
    'cashier.identifierLabel': 'Número do cliente',
    'cashier.identifierPlaceholder': 'Informe o identificador (ex: 42)',
    'cashier.identifierHelper': 'Confirme o número exibido no crachá do cliente.',
    'cashier.nameLabel': 'Nome do cliente',
    'cashier.namePlaceholder': 'Ex: Maria Silva',
    'cashier.availableFlavors': 'Sabores disponíveis',
    'cashier.refresh': 'Atualizar estoque',
    'cashier.loading': 'Carregando sabores disponíveis...',
    'cashier.error': 'Erro ao consultar sabores: {{message}}',
    'cashier.inStock': 'Em estoque: {{quantity}}',
    'cashier.totalPastels': 'Total de pastéis: {{count}}',
    'cashier.totalAmount': 'Valor total: {{value}}',
    'cashier.submit': 'Registrar pedido',
    'cashier.submitting': 'Registrando...',
    'cashier.successModal': 'Pedido registrado com sucesso!',
    'cashier.validation': 'Informe o número e o nome do cliente e selecione pelo menos um pastel.',
    'cashier.failure': 'Não foi possível registrar o pedido. Tente novamente.',
    'adminPastels.title': 'Cadastro de sabores de pastel',
    'adminPastels.subtitle': 'Monte uma lista de sabores para registrar de uma só vez e confirme antes de enviar.',
    'adminPastels.addFlavor': 'Adicionar sabor',
    'adminPastels.removeFlavor': 'Remover sabor',
    'adminPastels.nameLabel': 'Nome do sabor',
    'adminPastels.descriptionLabel': 'Descrição (opcional)',
    'adminPastels.imageLabel': 'Link da imagem (opcional)',
    'adminPastels.quantityLabel': 'Quantidade disponível',
    'adminPastels.priceLabel': 'Preço (R$)',
    'adminPastels.reviewButton': 'Revisar cadastro',
    'adminPastels.reviewTitle': 'Revise os sabores antes de confirmar',
    'adminPastels.reviewDescription': 'Confira os dados dos sabores cadastrados e confirme para registrar no sistema.',
    'adminPastels.reviewQuantity': 'Quantidade',
    'adminPastels.reviewPrice': 'Preço',
    'adminPastels.editButton': 'Editar pedido',
    'adminPastels.confirmButton': 'Confirmar cadastro',
    'adminPastels.submitting': 'Registrando...',
    'adminPastels.validationItems': 'Adicione pelo menos um sabor para continuar.',
    'adminPastels.validationName': 'Informe o nome do sabor.',
    'adminPastels.validationDuplicate': 'Já existe um sabor com o nome {{name}} na lista.',
    'adminPastels.validationQuantity': 'Informe uma quantidade válida (zero ou mais).',
    'adminPastels.validationPrice': 'Informe um preço válido para o sabor.',
    'adminPastels.success': '{{count}} sabor cadastrado com sucesso.',
    'adminPastels.success_plural': '{{count}} sabores cadastrados com sucesso.',
    'adminPastels.successItem': '{{name}} — {{quantity}} unidades',
    'adminPastels.error': 'Não foi possível registrar os sabores. Tente novamente.',
    'home.welcome': 'Bem-vindo ao Stand Manager',
    'home.description':
      'Centralize o fluxo dos pedidos de pastel da igreja em um único lugar. Acompanhe o estoque, receba pedidos em tempo real e coordene cada etapa da produção.',
    'home.ctaSelfService': 'Fazer pedido pelo QR Code',
    'home.ctaCashier': 'Registrar pedido no caixa',
    'home.ctaVolunteers': 'Painel dos voluntários',
    'home.feature1': 'Pedidos agrupados por cliente para facilitar a entrega.',
    'home.feature2': 'Atualizações visuais de status para cada etapa da produção.',
    'home.feature3': 'Estoque atualizado automaticamente conforme os pedidos são registrados.',
    'customerOrders.title': 'Meus pedidos',
    'customerOrders.noIdentity': 'Procure um voluntário para confirmar seu identificador e acompanhar seus pedidos aqui.',
    'customerOrders.identityLabel': 'Cliente #{{id}} — {{name}}',
    'customerOrders.refresh': 'Atualizar pedidos',
    'customerOrders.loading': 'Buscando seus pedidos...',
    'customerOrders.error': 'Não foi possível carregar seus pedidos: {{message}}',
    'customerOrders.empty': 'Você ainda não tem pedidos registrados.',
    'customerOrders.orderNumber': 'Pedido #{{order}}',
    'customerOrders.createdAt': 'Realizado em {{time}}',
    'customerOrders.totalAmountLabel': 'Valor total',
    'customerOrders.quantity': 'Quantidade: {{quantity}}',
    'customerOrders.unitPrice': 'Preço unitário: {{value}}',
    'customerOrders.timeline': 'Linha do tempo',
    'customerOrders.cancel': 'Cancelar pedido',
    'customerOrders.cancelling': 'Cancelando...',
    'customerOrders.cancelled': 'Pedido cancelado.',
    'customerOrders.cancelError': 'Não foi possível cancelar este pedido.',
    'customerOrders.lastUpdated': 'Atualizado às {{time}}',
    'volunteer.orderTotal': 'Valor do pedido: {{value}}',
    'volunteerHistory.title': 'Histórico de pedidos',
    'volunteerHistory.subtitle': 'Acompanhe a linha do tempo dos pedidos já finalizados ou em finalização.',
    'volunteerHistory.refresh': 'Atualizar histórico',
    'volunteerHistory.loading': 'Carregando histórico de pedidos...',
    'volunteerHistory.error': 'Erro ao carregar histórico: {{message}}',
    'volunteerHistory.empty': 'Nenhum pedido finalizado até o momento.',
    'volunteerHistory.identifier': 'Cliente #{{id}}',
    'volunteerHistory.searchPlaceholder': 'Filtrar histórico por nome ou número',
    'volunteerHistory.orderNumber': 'Pedido #{{order}}',
    'volunteerHistory.createdAt': 'Criado em {{time}}',
    'volunteerHistory.totalAmountLabel': 'Valor total',
    'volunteerHistory.quantity': 'Quantidade: {{quantity}}',
    'volunteerHistory.unitPrice': 'Preço unitário: {{value}}',
    'volunteerHistory.timeline': 'Linha do tempo',
    'volunteerHistory.completedItems': '{{count}} item no histórico',
    'volunteerHistory.completedItems_plural': '{{count}} itens no histórico',
    'reports.title': 'Relatórios do evento',
    'reports.subtitle': 'Acompanhe vendas, fluxo de pedidos e gargalos do dia.',
    'reports.dateLabel': 'Escolha a data',
    'reports.loading': 'Carregando relatório...',
    'reports.loadError': 'Não foi possível carregar o relatório.',
    'reports.summaryTitle': 'Resumo geral do dia',
    'reports.totalRevenue': 'Faturamento total',
    'reports.totalOrders': 'Pedidos registrados',
    'reports.totalItems': 'Pastéis vendidos',
    'reports.averageTicket': 'Ticket médio',
    'reports.averageCompletion': 'Tempo médio para concluir um pedido',
    'reports.noCompletionData': 'Sem dados suficientes',
    'reports.hourlyChartTitle': 'Picos de pedidos por hora',
    'reports.hourlyChartSubtitle': 'Visualize os horários de maior movimento.',
    'reports.noOrders': 'Nenhum pedido registrado nesse dia.',
    'reports.popularFlavorsTitle': 'Sabores mais pedidos',
    'reports.noFlavorData': 'Ainda não há dados de sabores para mostrar.',
    'reports.flavorColumn': 'Sabor',
    'reports.quantityColumn': 'Quantidade',
    'reports.revenueColumn': 'Faturamento',
    'reports.stepDurationsTitle': 'Etapas mais demoradas',
    'reports.noStepData': 'Ainda não há dados de duração por etapa.',
    'reports.stepColumn': 'Etapa',
    'reports.averageColumn': 'Média',
    'reports.fastestColumn': 'Mais rápido',
    'reports.slowestColumn': 'Mais demorado',
    'reports.durationFormat': '{{seconds}}s',
    'api.errorDefault': 'Erro ao comunicar com a API ({{status}})'
  },
  'en-US': {
    'app.brand': 'Stand Manager',
    'footer.communityProject': 'Community project - Pastel order manager',
    'nav.home': 'Home',
    'nav.volunteers': 'Active Orders',
    'nav.volunteerHistory': 'History',
    'nav.cashier': 'Cashier',
    'nav.myOrders': 'My orders',
    'nav.selfService': 'Self-service',
    'nav.admin': 'Admin',
    'nav.reports': 'Reports',
    'identity.headerBadge': 'Customer #{{id}} — {{name}}',
    'identity.askRoleTitle': 'Are you a volunteer at the event?',
    'identity.askRoleSubtitle': 'We use this information to unlock the right screens for you.',
    'identity.imVolunteer': 'I am a volunteer',
    'identity.imGuest': 'I am attending as a guest',
    'identity.hasIdentifierQuestion': 'I already have an identifier number',
    'identity.askIdentifierTitle': 'Do you already have an identifier number?',
    'identity.hasIdentifierYes': 'Yes, I already know my number',
    'identity.hasIdentifierNo': 'No, I need a new number',
    'identity.enterIdentifierTitle': 'Enter your identifier number',
    'identity.identifierPlaceholder': 'Ex: 42',
    'identity.loading': 'Loading…',
    'identity.continue': 'Continue',
    'identity.confirmNameTitle': 'Confirm your name — Customer #{{id}}',
    'identity.confirmNameSubtitle': 'We have {{name}} on record. Type it exactly the same to confirm.',
    'identity.namePlaceholder': 'Type your full name',
    'identity.confirmNameButton': 'Confirm name',
    'identity.identifierInvalid': 'We could not find that identifier. Try again or speak with a volunteer.',
    'identity.nameRequired': 'Please provide your name to continue.',
    'identity.nameMismatch': 'The name does not match our records. Please ask a volunteer for help.',
    'identity.enterNameTitle': 'Let’s create your identifier',
    'identity.enterNameSubtitle': 'Tell us your full name and we will generate an easy number for you.',
    'identity.createIdentifierButton': 'Generate identifier number',
    'identity.alreadyHaveIdentifier': 'I already have an identifier number',
    'identity.registerFailed': 'We could not generate the identifier. Please try again.',
    'identity.customerWelcome': 'Great! You are customer number {{id}}, {{name}}!',
    'identity.persistReminder': 'This number will stay in the header on the left so you never forget it.',
    'languageSelector.label': 'Select language',
    'language.pt-BR': 'Portuguese (Brazil)',
    'language.en-US': 'English (US)',
    'language.es-ES': 'Spanish',
    'volunteer.title': 'Volunteer board',
    'volunteer.pendingItems': 'Pending items',
    'volunteer.refresh': 'Refresh now',
    'volunteer.success': 'Order updated successfully.',
    'volunteer.genericError': 'Unable to update the order.',
    'volunteer.loading': 'Loading orders in progress...',
    'volunteer.error': 'Failed to load orders: {{message}}',
    'volunteer.empty': 'No pending orders right now. 🎉',
    'volunteer.identifier': 'Customer #{{id}}',
    'volunteer.searchPlaceholder': 'Filter by customer name or number',
    'volunteer.pendingPastels': '{{count}} pastel',
    'volunteer.pendingPastels_plural': '{{count}} pastels',
    'volunteer.orderNumber': 'Order #{{order}}',
    'volunteer.receivedAt': 'Received at {{time}}',
    'volunteer.unitLabel': 'Unit {{unit}} of {{total}}',
    'volunteer.advance': 'Advance step',
    'volunteer.completed': 'Completed',
    'volunteer.finalizedMessage': 'Order completed',
    'orderStatus.pending': 'Waiting',
    'orderStatus.frying': 'Frying',
    'orderStatus.readyForPickup': 'Ready',
    'orderStatus.finalized': 'Order completed',
    'orderStatus.cancelled': 'Cancelled',
    'orderStatus.progress': 'Step {{step}} of {{total}}',
    'orderStatus.unknown': 'Unknown step',
    'customerOrder.title': 'Order your pastel',
    'customerOrder.subtitle':
      'Choose the flavors available in real time and send your order. We will let you know when it is ready! 😊',
    'customerOrder.customerBadge': 'Customer #{{id}} — {{name}}',
    'customerOrder.identityMissing': 'Please talk to a volunteer so we can confirm your identifier before placing orders.',
    'customerOrder.nameLabel': 'Your name',
    'customerOrder.namePlaceholder': 'How should we call you?',
    'customerOrder.availableFlavors': 'Available flavors',
    'customerOrder.refresh': 'Refresh list',
    'customerOrder.loading': 'Loading delicious flavors...',
    'customerOrder.error': 'Unable to load flavors: {{message}}',
    'customerOrder.inStock': 'Available: {{quantity}}',
    'customerOrder.itemsInCart': 'Items in cart: {{count}}',
    'customerOrder.totalAmount': 'Total: {{value}}',
    'customerOrder.submit': 'Place order',
    'customerOrder.submitting': 'Sending...',
    'customerOrder.success': 'Order sent! You will receive a notification when it is ready.',
    'customerOrder.validation': 'Choose at least one flavor to continue.',
    'customerOrder.failure': 'We could not register your order. Please try again.',
    'cashier.title': 'Order registration - Cashier',
    'cashier.subtitle':
      'Check the live stock and quickly select the flavors requested by participants.',
    'cashier.identifierLabel': 'Customer number',
    'cashier.identifierPlaceholder': 'Type the identifier (e.g., 42)',
    'cashier.identifierHelper': 'Confirm the number shown on the customer badge.',
    'cashier.nameLabel': 'Customer name',
    'cashier.namePlaceholder': 'E.g.: Mary Smith',
    'cashier.availableFlavors': 'Available flavors',
    'cashier.refresh': 'Refresh stock',
    'cashier.loading': 'Loading available flavors...',
    'cashier.error': 'Failed to fetch flavors: {{message}}',
    'cashier.inStock': 'In stock: {{quantity}}',
    'cashier.totalPastels': 'Total pastels: {{count}}',
    'cashier.totalAmount': 'Order amount: {{value}}',
    'cashier.submit': 'Register order',
    'cashier.submitting': 'Registering...',
    'cashier.successModal': 'Order registered successfully!',
    'cashier.validation': 'Provide the customer number and name, and select at least one pastel.',
    'cashier.failure': 'Unable to register the order. Please try again.',
    'adminPastels.title': 'Register pastel flavors',
    'adminPastels.subtitle': 'Prepare a batch of flavors, review the details, and confirm before saving.',
    'adminPastels.addFlavor': 'Add flavor',
    'adminPastels.removeFlavor': 'Remove flavor',
    'adminPastels.nameLabel': 'Flavor name',
    'adminPastels.descriptionLabel': 'Description (optional)',
    'adminPastels.imageLabel': 'Image link (optional)',
    'adminPastels.quantityLabel': 'Available quantity',
    'adminPastels.priceLabel': 'Price (BRL)',
    'adminPastels.reviewButton': 'Review batch',
    'adminPastels.reviewTitle': 'Review the flavors before confirming',
    'adminPastels.reviewDescription': 'Double-check the flavors that will be created and confirm to save them.',
    'adminPastels.reviewQuantity': 'Quantity',
    'adminPastels.reviewPrice': 'Price',
    'adminPastels.editButton': 'Edit order',
    'adminPastels.confirmButton': 'Confirm registration',
    'adminPastels.submitting': 'Saving...',
    'adminPastels.validationItems': 'Add at least one flavor to continue.',
    'adminPastels.validationName': 'Provide the flavor name.',
    'adminPastels.validationDuplicate': 'There is already a flavor named {{name}} in this list.',
    'adminPastels.validationQuantity': 'Enter a valid quantity (zero or more).',
    'adminPastels.validationPrice': 'Enter a valid price for the flavor.',
    'adminPastels.success': '{{count}} flavor registered successfully.',
    'adminPastels.success_plural': '{{count}} flavors registered successfully.',
    'adminPastels.successItem': '{{name}} — {{quantity}} units',
    'adminPastels.error': 'We could not register the flavors. Please try again.',
    'home.welcome': 'Welcome to Stand Manager',
    'home.description':
      'Centralize your church pastel orders in a single place. Track stock, receive orders in real time, and coordinate every production step.',
    'home.ctaSelfService': 'Order via QR Code',
    'home.ctaCashier': 'Register order at the cashier',
    'home.ctaVolunteers': 'Volunteer board',
    'home.feature1': 'Orders grouped by customer to simplify delivery.',
    'home.feature2': 'Visual status updates for every production step.',
    'home.feature3': 'Stock automatically updated as orders are registered.',
    'customerOrders.title': 'My orders',
    'customerOrders.noIdentity': 'Talk to a volunteer to confirm your identifier and see your orders here.',
    'customerOrders.identityLabel': 'Customer #{{id}} — {{name}}',
    'customerOrders.refresh': 'Refresh orders',
    'customerOrders.loading': 'Fetching your orders...',
    'customerOrders.error': 'Could not load your orders: {{message}}',
    'customerOrders.empty': 'You do not have orders yet.',
    'customerOrders.orderNumber': 'Order #{{order}}',
    'customerOrders.createdAt': 'Placed at {{time}}',
    'customerOrders.totalAmountLabel': 'Total amount',
    'customerOrders.quantity': 'Quantity: {{quantity}}',
    'customerOrders.unitPrice': 'Unit price: {{value}}',
    'customerOrders.timeline': 'Timeline',
    'customerOrders.cancel': 'Cancel order',
    'customerOrders.cancelling': 'Cancelling...',
    'customerOrders.cancelled': 'Order cancelled.',
    'customerOrders.cancelError': 'We could not cancel this order.',
    'customerOrders.lastUpdated': 'Updated at {{time}}',
    'volunteer.orderTotal': 'Order amount: {{value}}',
    'volunteerHistory.title': 'Order history',
    'volunteerHistory.subtitle': 'Track the timeline of completed orders and those wrapping up.',
    'volunteerHistory.refresh': 'Refresh history',
    'volunteerHistory.loading': 'Loading order history...',
    'volunteerHistory.error': 'Could not load history: {{message}}',
    'volunteerHistory.empty': 'No completed orders yet.',
    'volunteerHistory.identifier': 'Customer #{{id}}',
    'volunteerHistory.searchPlaceholder': 'Filter history by name or number',
    'volunteerHistory.orderNumber': 'Order #{{order}}',
    'volunteerHistory.createdAt': 'Created at {{time}}',
    'volunteerHistory.totalAmountLabel': 'Total amount',
    'volunteerHistory.quantity': 'Quantity: {{quantity}}',
    'volunteerHistory.unitPrice': 'Unit price: {{value}}',
    'volunteerHistory.timeline': 'Timeline',
    'volunteerHistory.completedItems': '{{count}} item in history',
    'volunteerHistory.completedItems_plural': '{{count}} items in history',
    'reports.title': 'Event reports',
    'reports.subtitle': 'Track revenue, order flow, and bottlenecks for the day.',
    'reports.dateLabel': 'Select the date',
    'reports.loading': 'Loading report...',
    'reports.loadError': 'Unable to load the report.',
    'reports.summaryTitle': 'Daily overview',
    'reports.totalRevenue': 'Total revenue',
    'reports.totalOrders': 'Orders placed',
    'reports.totalItems': 'Pastels sold',
    'reports.averageTicket': 'Average ticket',
    'reports.averageCompletion': 'Average completion time',
    'reports.noCompletionData': 'Not enough data',
    'reports.hourlyChartTitle': 'Orders by hour',
    'reports.hourlyChartSubtitle': 'See peak activity throughout the day.',
    'reports.noOrders': 'No orders were recorded on this day.',
    'reports.popularFlavorsTitle': 'Most popular flavors',
    'reports.noFlavorData': 'No flavor data is available yet.',
    'reports.flavorColumn': 'Flavor',
    'reports.quantityColumn': 'Quantity',
    'reports.revenueColumn': 'Revenue',
    'reports.stepDurationsTitle': 'Longest steps',
    'reports.noStepData': 'No step duration data yet.',
    'reports.stepColumn': 'Step',
    'reports.averageColumn': 'Average',
    'reports.fastestColumn': 'Fastest',
    'reports.slowestColumn': 'Slowest',
    'reports.durationFormat': '{{seconds}}s',
    'api.errorDefault': 'Error communicating with the API ({{status}})'
  },
  'es-ES': {
    'app.brand': 'Stand Manager',
    'footer.communityProject': 'Proyecto comunitario - Gestor de pedidos de pastel',
    'nav.home': 'Inicio',
    'nav.volunteers': 'Pedidos activos',
    'nav.volunteerHistory': 'Historial',
    'nav.cashier': 'Caja',
    'nav.myOrders': 'Mis pedidos',
    'nav.selfService': 'Autoservicio',
    'nav.admin': 'Administración',
    'nav.reports': 'Informes',
    'identity.headerBadge': 'Cliente #{{id}} — {{name}}',
    'identity.askRoleTitle': '¿Eres voluntario en el evento?',
    'identity.askRoleSubtitle': 'Usamos esta información para mostrarte las pantallas correctas.',
    'identity.imVolunteer': 'Soy voluntario',
    'identity.imGuest': 'Soy cliente',
    'identity.hasIdentifierQuestion': 'Ya tengo un número identificador',
    'identity.askIdentifierTitle': '¿Ya tienes un número identificador?',
    'identity.hasIdentifierYes': 'Sí, ya conozco mi número',
    'identity.hasIdentifierNo': 'No, necesito un número nuevo',
    'identity.enterIdentifierTitle': 'Ingresa tu número identificador',
    'identity.identifierPlaceholder': 'Ej: 42',
    'identity.loading': 'Cargando…',
    'identity.continue': 'Continuar',
    'identity.confirmNameTitle': 'Confirma tu nombre — Cliente #{{id}}',
    'identity.confirmNameSubtitle': 'Registramos {{name}}. Escríbelo exactamente igual para confirmar.',
    'identity.namePlaceholder': 'Escribe tu nombre completo',
    'identity.confirmNameButton': 'Confirmar nombre',
    'identity.identifierInvalid': 'No encontramos ese identificador. Intenta de nuevo o busca a un voluntario.',
    'identity.nameRequired': 'Ingresa tu nombre para continuar.',
    'identity.nameMismatch': 'El nombre no coincide con el registro. Solicita ayuda a un voluntario.',
    'identity.enterNameTitle': 'Vamos a crear tu identificador',
    'identity.enterNameSubtitle': 'Cuéntanos tu nombre completo y generaremos un número fácil de recordar.',
    'identity.createIdentifierButton': 'Generar número identificador',
    'identity.alreadyHaveIdentifier': 'Ya tengo un número identificador',
    'identity.registerFailed': 'No pudimos generar el identificador. Intenta nuevamente.',
    'identity.customerWelcome': '¡Listo! Eres el cliente número {{id}}, {{name}}!',
    'identity.persistReminder': 'Este número quedará siempre en la esquina izquierda del encabezado para que no lo olvides.',
    'languageSelector.label': 'Seleccionar idioma',
    'language.pt-BR': 'Portugués (Brasil)',
    'language.en-US': 'Inglés (EE. UU.)',
    'language.es-ES': 'Español',
    'volunteer.title': 'Panel de voluntarios',
    'volunteer.pendingItems': 'Artículos pendientes',
    'volunteer.refresh': 'Actualizar ahora',
    'volunteer.success': 'Pedido actualizado correctamente.',
    'volunteer.genericError': 'No se pudo actualizar el pedido.',
    'volunteer.loading': 'Cargando pedidos en progreso...',
    'volunteer.error': 'Error al cargar los pedidos: {{message}}',
    'volunteer.empty': 'No hay pedidos pendientes en este momento. 🎉',
    'volunteer.identifier': 'Cliente #{{id}}',
    'volunteer.searchPlaceholder': 'Filtrar por nombre o número del cliente',
    'volunteer.pendingPastels': '{{count}} pastel',
    'volunteer.pendingPastels_plural': '{{count}} pasteles',
    'volunteer.orderNumber': 'Pedido #{{order}}',
    'volunteer.receivedAt': 'Recibido a las {{time}}',
    'volunteer.unitLabel': 'Unidad {{unit}} de {{total}}',
    'volunteer.advance': 'Avanzar etapa',
    'volunteer.completed': 'Completado',
    'volunteer.finalizedMessage': 'Pedido finalizado',
    'orderStatus.pending': 'En espera',
    'orderStatus.frying': 'Friéndose',
    'orderStatus.readyForPickup': 'Listo',
    'orderStatus.finalized': 'Pedido finalizado',
    'orderStatus.cancelled': 'Cancelado',
    'orderStatus.progress': 'Etapa {{step}} de {{total}}',
    'orderStatus.unknown': 'Etapa desconocida',
    'customerOrder.title': 'Pide tu pastel',
    'customerOrder.subtitle':
      'Elige los sabores disponibles en tiempo real y envía tu pedido. ¡Te avisaremos cuando esté listo! 😊',
    'customerOrder.customerBadge': 'Cliente #{{id}} — {{name}}',
    'customerOrder.identityMissing': 'Habla con un voluntario para confirmar tu identificador antes de hacer pedidos.',
    'customerOrder.nameLabel': 'Tu nombre',
    'customerOrder.namePlaceholder': '¿Cómo debemos llamarte?',
    'customerOrder.availableFlavors': 'Sabores disponibles',
    'customerOrder.refresh': 'Actualizar lista',
    'customerOrder.loading': 'Cargando sabores deliciosos...',
    'customerOrder.error': 'No se pudieron cargar los sabores: {{message}}',
    'customerOrder.inStock': 'Disponibles: {{quantity}}',
    'customerOrder.itemsInCart': 'Artículos en el carrito: {{count}}',
    'customerOrder.totalAmount': 'Total: {{value}}',
    'customerOrder.submit': 'Finalizar pedido',
    'customerOrder.submitting': 'Enviando...',
    'customerOrder.success': '¡Pedido enviado! Recibirás una notificación cuando esté listo.',
    'customerOrder.validation': 'Elige tus sabores e informa tu nombre para finalizar.',
    'customerOrder.failure': 'No pudimos registrar tu pedido. Inténtalo de nuevo.',
    'cashier.title': 'Registro de pedidos - Caja',
    'cashier.subtitle':
      'Consulta el inventario actualizado y selecciona rápidamente los sabores solicitados por los participantes.',
    'cashier.identifierLabel': 'Número del cliente',
    'cashier.identifierPlaceholder': 'Escribe el identificador (ej.: 42)',
    'cashier.identifierHelper': 'Confirma el número que aparece en la credencial del cliente.',
    'cashier.nameLabel': 'Nombre del cliente',
    'cashier.namePlaceholder': 'Ej.: María López',
    'cashier.availableFlavors': 'Sabores disponibles',
    'cashier.refresh': 'Actualizar inventario',
    'cashier.loading': 'Cargando sabores disponibles...',
    'cashier.error': 'Error al consultar los sabores: {{message}}',
    'cashier.inStock': 'En inventario: {{quantity}}',
    'cashier.totalPastels': 'Total de pasteles: {{count}}',
    'cashier.totalAmount': 'Valor total: {{value}}',
    'cashier.submit': 'Registrar pedido',
    'cashier.submitting': 'Registrando...',
    'cashier.successModal': '¡Pedido registrado con éxito!',
    'cashier.validation': 'Indica el número y el nombre del cliente y selecciona al menos un pastel.',
    'cashier.failure': 'No se pudo registrar el pedido. Inténtalo de nuevo.',
    'adminPastels.title': 'Registrar sabores de pastel',
    'adminPastels.subtitle': 'Prepara un lote de sabores, revísalos y confirma antes de guardarlos.',
    'adminPastels.addFlavor': 'Agregar sabor',
    'adminPastels.removeFlavor': 'Eliminar sabor',
    'adminPastels.nameLabel': 'Nombre del sabor',
    'adminPastels.descriptionLabel': 'Descripción (opcional)',
    'adminPastels.imageLabel': 'Enlace de la imagen (opcional)',
    'adminPastels.quantityLabel': 'Cantidad disponible',
    'adminPastels.priceLabel': 'Precio (BRL)',
    'adminPastels.reviewButton': 'Revisar registro',
    'adminPastels.reviewTitle': 'Revisa los sabores antes de confirmar',
    'adminPastels.reviewDescription': 'Verifica la información de los sabores y confirma para guardarlos.',
    'adminPastels.reviewQuantity': 'Cantidad',
    'adminPastels.reviewPrice': 'Precio',
    'adminPastels.editButton': 'Editar pedido',
    'adminPastels.confirmButton': 'Confirmar registro',
    'adminPastels.submitting': 'Guardando...',
    'adminPastels.validationItems': 'Agrega al menos un sabor para continuar.',
    'adminPastels.validationName': 'Indica el nombre del sabor.',
    'adminPastels.validationDuplicate': 'Ya existe un sabor llamado {{name}} en la lista.',
    'adminPastels.validationQuantity': 'Ingresa una cantidad válida (cero o más).',
    'adminPastels.validationPrice': 'Ingresa un precio válido para el sabor.',
    'adminPastels.success': '{{count}} sabor registrado con éxito.',
    'adminPastels.success_plural': '{{count}} sabores registrados con éxito.',
    'adminPastels.successItem': '{{name}} — {{quantity}} unidades',
    'adminPastels.error': 'No pudimos registrar los sabores. Inténtalo de nuevo.',
    'home.welcome': 'Bienvenido a Stand Manager',
    'home.description':
      'Centraliza el flujo de pedidos de pastel de la iglesia en un solo lugar. Controla el inventario, recibe pedidos en tiempo real y coordina cada etapa de la producción.',
    'home.ctaSelfService': 'Hacer pedido con el código QR',
    'home.ctaCashier': 'Registrar pedido en la caja',
    'home.ctaVolunteers': 'Panel de voluntarios',
    'home.feature1': 'Pedidos agrupados por cliente para facilitar la entrega.',
    'home.feature2': 'Actualizaciones visuales de estado para cada etapa de la producción.',
    'home.feature3': 'Inventario actualizado automáticamente a medida que se registran los pedidos.',
    'customerOrders.title': 'Mis pedidos',
    'customerOrders.noIdentity': 'Habla con un voluntario para confirmar tu identificador y ver tus pedidos aquí.',
    'customerOrders.identityLabel': 'Cliente #{{id}} — {{name}}',
    'customerOrders.refresh': 'Actualizar pedidos',
    'customerOrders.loading': 'Buscando tus pedidos...',
    'customerOrders.error': 'No se pudieron cargar tus pedidos: {{message}}',
    'customerOrders.empty': 'Aún no tienes pedidos registrados.',
    'customerOrders.orderNumber': 'Pedido #{{order}}',
    'customerOrders.createdAt': 'Realizado a las {{time}}',
    'customerOrders.totalAmountLabel': 'Valor total',
    'customerOrders.quantity': 'Cantidad: {{quantity}}',
    'customerOrders.unitPrice': 'Precio unitario: {{value}}',
    'customerOrders.timeline': 'Línea de tiempo',
    'customerOrders.cancel': 'Cancelar pedido',
    'customerOrders.cancelling': 'Cancelando...',
    'customerOrders.cancelled': 'Pedido cancelado.',
    'customerOrders.cancelError': 'No fue posible cancelar este pedido.',
    'customerOrders.lastUpdated': 'Actualizado a las {{time}}',
    'volunteer.orderTotal': 'Valor del pedido: {{value}}',
    'volunteerHistory.title': 'Historial de pedidos',
    'volunteerHistory.subtitle': 'Consulta la línea de tiempo de los pedidos finalizados o en finalización.',
    'volunteerHistory.refresh': 'Actualizar historial',
    'volunteerHistory.loading': 'Cargando historial de pedidos...',
    'volunteerHistory.error': 'No se pudo cargar el historial: {{message}}',
    'volunteerHistory.empty': 'Aún no hay pedidos finalizados.',
    'volunteerHistory.identifier': 'Cliente #{{id}}',
    'volunteerHistory.searchPlaceholder': 'Filtrar historial por nombre o número',
    'volunteerHistory.orderNumber': 'Pedido #{{order}}',
    'volunteerHistory.createdAt': 'Creado el {{time}}',
    'volunteerHistory.totalAmountLabel': 'Valor total',
    'volunteerHistory.quantity': 'Cantidad: {{quantity}}',
    'volunteerHistory.unitPrice': 'Precio unitario: {{value}}',
    'volunteerHistory.timeline': 'Línea de tiempo',
    'volunteerHistory.completedItems': '{{count}} artículo en el historial',
    'volunteerHistory.completedItems_plural': '{{count}} artículos en el historial',
    'reports.title': 'Informes del evento',
    'reports.subtitle': 'Consulta las ventas, el flujo de pedidos y los cuellos de botella del día.',
    'reports.dateLabel': 'Elige la fecha',
    'reports.loading': 'Cargando informe...',
    'reports.loadError': 'No se pudo cargar el informe.',
    'reports.summaryTitle': 'Resumen del día',
    'reports.totalRevenue': 'Facturación total',
    'reports.totalOrders': 'Pedidos registrados',
    'reports.totalItems': 'Pasteles vendidos',
    'reports.averageTicket': 'Ticket promedio',
    'reports.averageCompletion': 'Tiempo medio de finalización',
    'reports.noCompletionData': 'Datos insuficientes',
    'reports.hourlyChartTitle': 'Pedidos por hora',
    'reports.hourlyChartSubtitle': 'Visualiza los horarios de mayor movimiento.',
    'reports.noOrders': 'No se registraron pedidos en este día.',
    'reports.popularFlavorsTitle': 'Sabores más pedidos',
    'reports.noFlavorData': 'Aún no hay datos de sabores para mostrar.',
    'reports.flavorColumn': 'Sabor',
    'reports.quantityColumn': 'Cantidad',
    'reports.revenueColumn': 'Facturación',
    'reports.stepDurationsTitle': 'Etapas más lentas',
    'reports.noStepData': 'Aún no hay datos de duración por etapa.',
    'reports.stepColumn': 'Etapa',
    'reports.averageColumn': 'Promedio',
    'reports.fastestColumn': 'Más rápido',
    'reports.slowestColumn': 'Más lento',
    'reports.durationFormat': '{{seconds}}s',
    'api.errorDefault': 'Error al comunicarse con la API ({{status}})'
  }
};

const listeners = new Set<(language: LanguageCode) => void>();

function detectInitialLanguage(): LanguageCode {
  if (typeof window === 'undefined') {
    return DEFAULT_LANGUAGE;
  }

  const stored = window.localStorage.getItem(STORAGE_KEY) as LanguageCode | null;
  if (stored && TRANSLATIONS[stored]) {
    return stored;
  }

  const browserLanguages = window.navigator.languages ?? [window.navigator.language];
  const availableCodes = new Set<LanguageCode>(Object.keys(TRANSLATIONS) as LanguageCode[]);
  for (const language of browserLanguages) {
    const normalized = language.toLowerCase();
    for (const option of availableCodes) {
      if (option.toLowerCase() === normalized) {
        return option;
      }

      if (normalized.startsWith(option.split('-')[0].toLowerCase())) {
        return option;
      }
    }
  }

  return DEFAULT_LANGUAGE;
}

let currentLanguage: LanguageCode = detectInitialLanguage();

export function getCurrentLanguage(): LanguageCode {
  return currentLanguage;
}

export function setCurrentLanguage(language: LanguageCode): void {
  if (!TRANSLATIONS[language] || language === currentLanguage) {
    return;
  }

  currentLanguage = language;

  if (typeof window !== 'undefined') {
    window.localStorage.setItem(STORAGE_KEY, language);
  }

  listeners.forEach((listener) => listener(language));
}

export function subscribeLanguage(listener: (language: LanguageCode) => void): () => void {
  listeners.add(listener);
  return () => listeners.delete(listener);
}

export function getLanguageOptions(): LanguageOption[] {
  return LANGUAGE_OPTIONS;
}

function resolveDictionary(language: LanguageCode): TranslationDictionary {
  return TRANSLATIONS[language] ?? TRANSLATIONS[DEFAULT_LANGUAGE];
}

function formatTemplate(template: string, params?: TranslateParams): string {
  if (!params) {
    return template;
  }

  return template.replace(/\{\{(\w+)\}\}/g, (_, token: string) => {
    const value = params[token];
    return value === undefined ? '' : String(value);
  });
}

export function translate(key: string, params?: TranslateParams, language: LanguageCode = currentLanguage): string {
  const dictionary = resolveDictionary(language);
  let resolvedKey = key;

  if (params?.count !== undefined) {
    const pluralKey = `${key}_plural`;
    if (params.count !== 1 && (dictionary[pluralKey] || resolveDictionary(DEFAULT_LANGUAGE)[pluralKey])) {
      resolvedKey = pluralKey;
    }
  }

  const template =
    dictionary[resolvedKey] ??
    resolveDictionary(DEFAULT_LANGUAGE)[resolvedKey] ??
    dictionary[key] ??
    resolveDictionary(DEFAULT_LANGUAGE)[key] ??
    key;

  return formatTemplate(template, params);
}

